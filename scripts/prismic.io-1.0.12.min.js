(function (Global, undefined) {
    ...
    prismic.fn = prismic.prototype = {
        ...
        previewBar: function() {
            var currentRef = (/ref=([a-zA-Z0-9_-]+)/.exec(document.location.search) || [])[1];
            var refs = this.data.refs;
            return '<select onchange="document.location = \'/?ref=\' + this.value">' + (function() {
                return refs.map(function(r) {
                    return '<option value="' + r.ref + '" ' + (r.ref == currentRef ? 'selected="true"' : '') + '>' + (r.isMaster ? 'As guest viewer' : 'Preview ' + r.label) + '</option>';
                }).join('');
            })() + '</select>';
        },
    ...
    SearchForm.prototype = {
        ...
        query: function(query) {
            if(typeof query === 'string') {
                ...
                return this.query("[[:d = " +  query[0] + "(" + query[1] + ", " + (function(){
                    return query.slice(2).map(function(p) {
                        if(typeof p === 'string') {
                            return '"' + p + '"';
